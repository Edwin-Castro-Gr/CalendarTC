...
class AuthController extends BaseController
{
    ...
    public function resetPasswordHandler($token)
    {
        ...
        $userModel = new User();
        $user = $userModel->where('correo_usuario', $tokenData['email'])->first();

        if (!$user) {
            return redirect()->route('admin.forgot.form')->with('fail', 'No se encontró el usuario asociado a este correo electrónico.');
        }

        // Actualizar la contraseña del usuario
        $userModel-> where('id', $user['id'])->set([
            'password' => Hash::make($this->request->getVar('nuevo_password'))
        ])->update();

        // Eliminar el token utilizado
        $passwordResetToken->where('token', $token)->delete();

        return redirect()->route('admin.login.form')->with('success', 'Contraseña restablecida con éxito. Ahora puede iniciar sesión.');
    }
}
